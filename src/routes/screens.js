...
router.get('/status-by-id/:id', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, name, show_status, show_alerts, rotation, audio, transition, auto_start, auto_monitor FROM screens WHERE id = $1',
      [req.params.id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'Tela não encontrada' });
